...
This repository contains two separate experiences:

- The real local app runs with Next.js at `http://localhost:3000`.
- `index.html` is a standalone visual demo for GitHub Pages. It uses mock data and has no login, database, or uploads.
...
Run these commands from this folder. The setup command creates `data/sprout.db` and the local
upload directory. Both are excluded from Git by `.gitignore`.
...
For a local production-style run, use `npm run build` followed by `npm start` and open
`http://localhost:3000`. Do not use GitHub Pages for this version: GitHub Pages cannot run the
database, server actions, sessions, or protected document routes.
...
is to delete `data/sprout.db`, edit the password in `seed.js`, and run
`npm run setup` again.
